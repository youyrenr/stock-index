...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fetchKeys = async () => {
      try {
        const response = await api.post('/prompts/keys', { skip: 0, limit: 100 })
    ...
    const addNewKey = async () => {
      if (newKey.value.trim()) {
        try {
          await api.post('/prompts', { key: newKey.value, value: newKey.value })
          items.value.push({ key: newKey.value })
    ...
    const deleteKey = async () => {
      try {
        await api.delete('/prompts', { data: { key: keyToDelete.value } })
